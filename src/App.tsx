...
import { addContact, addEvidence, addLead, addNote, approveDraft, confirmSent, createBackup, deleteLead, emptyDashboardCounts, exportLeads, getDraftSeed, getFollowUps, getLatestDraft, getSettings, listBackups, loadContacts, loadDashboardCounts, loadLeads, loadReviewData, markReplied, openGmail, restoreBackup, saveDraft, setLeadStatus, snoozeFollowUp, updateLead, updateSetting, type BackupInfo, type FollowUpSummary } from "./lib/desktop-api";
    ...
    <div className="app-shell">
      ...
      <main>
        ...
        <section className="workspace">
          ...
          ) : activeView === "Settings" ? <SettingsView onNotice={setNotice} /> : activeView === "Follow-ups" ? <FollowUpsView onChanged={refresh} /> : (
...
function ReviewDialog({ lead, onClose, onChanged }: { lead: LeadSummary; onClose: () => void; onChanged: () => Promise<void> }) {
  ...
  return <div className="dialog-backdrop"><section className="dialog review-dialog" role="dialog" aria-modal="true"><div className="dialog-heading"><div><p className="eyebrow">Evidence review</p><h2>{lead.companyName}</h2><a className="source-link" href={lead.officialWebsite} target="_blank" rel="noreferrer">{lead.officialWebsite} <ExternalLink size={12}/></a></div><div className="heading-actions"><button className="secondary-action" onClick={() => setShowEdit(true)}>Edit lead</button><button className="icon-button" onClick={onClose}><X size={18}/></button></div></div><div className="review-grid"><div className="review-column"><h3>Add structured evidence</h3><form onSubmit={submitEvidence}>
...
function DraftDialog({ lead, contacts, onClose, onChanged }: { lead: LeadSummary; contacts: ContactRecord[]; onClose: () => void; onChanged: () => Promise<void> }) {
  ...
  async function launchGmail() { if (!draft) return; try { await openGmail(draft.id); setDraft({ ...draft, gmailOpenedAt: new Date().toISOString() }); await onChanged(); } catch (reason) { setError(String(reason)); } }
  async function recordSend(sent: boolean) { if (!draft) return; try { await confirmSent(draft.id, sent); await onChanged(); onClose(); } catch (reason) { setError(String(reason)); } }
  async function copyDraft() { try { await navigator.clipboard.writeText(`To: ${draft?.recipient || ""}\nSubject: ${subject}\n\n${body}`); setError(undefined); } catch { setError("Clipboard access failed. Select and copy the message manually."); } }
  const subject = draft?.subject ?? seed?.subject ?? ""; const body = draft?.body ?? seed?.body ?? "";
  return <div className="dialog-backdrop nested-dialog"><section className="dialog draft-dialog"><div className="dialog-heading"><div><p className="eyebrow">Human review required</p><h2>Outreach draft</h2></div><button className="icon-button" onClick={onClose}><X size={18}/></button></div><form key={draft?.updatedAt || seed?.subject || "loading"} onSubmit={submit}><label>Recipient<select name="contactId" defaultValue={draft?.contactId || contacts[0]?.id}>{contacts.map(contact => <option value={contact.id} key={contact.id}>{contact.value}{contact.cautionReason ? " — caution" : ""}</option>)}</select></label><label>Subject<input name="subject" required maxLength={150} defaultValue={subject}/></label><label>Message<textarea name="body" required minLength={20} maxLength={10000} defaultValue={body}/></label>{lead.language === "fr" && <label>Optional English translation for review<textarea name="englishTranslation" defaultValue={draft?.englishTranslation}/></label>}<div className="form-assurance">Nothing is sent automatically. Gmail opens a prepared compose window; you confirm the outcome afterward.</div>{error && <p className="form-error">{error}</p>}<div className="dialog-actions"><button type="button" className="secondary-action" onClick={onClose}>Close</button><button className="secondary-action">Save draft</button><button type="button" className="secondary-action" disabled={!draft} onClick={() => void copyDraft()}>Copy</button>{!draft?.approvedAt ? <button type="button" className="primary-action" disabled={!draft} onClick={() => void approve()}>Approve exact draft</button> : !draft.gmailOpenedAt ? <button type="button" className="primary-action" onClick={() => void launchGmail()}>Open in Gmail</button> : <><button type="button" className="secondary-action" onClick={() => void recordSend(false)}>Not sent</button><button type="button" className="primary-action" onClick={() => void recordSend(true)}>I sent it</button></>}</div></form></section></div>;
}

function EditLeadDialog({ lead, onClose, onSaved }: { lead: LeadSummary; onClose: () => void; onSaved: () => Promise<void> }) {
  const [error, setError] = useState<string>(); const [saving, setSaving] = useState(false);
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const values = new FormData(event.currentTarget); let website=String(values.get("officialWebsite")||"").trim(); if (!website.includes("://")) website=`https://${website}`; const parsed=leadInputSchema.safeParse({ companyName:values.get("companyName"), officialWebsite:website, country:values.get("country"), language:values.get("language"), cityOrServiceArea:String(values.get("cityOrServiceArea")||"")||undefined }); if (!parsed.success) { setError(parsed.error.issues[0]?.message); return; } try { setSaving(true); await updateLead({ id:lead.id, ...parsed.data, officialWebsite:normalizeOfficialWebsite(parsed.data.officialWebsite) }); await onSaved(); } catch (reason) { setError(String(reason)); } finally { setSaving(false); } }
  async function remove() { if (!["rejected","archived","do_not_contact"].includes(lead.status)) { setError("Move this lead to Rejected, Archived, or Do not contact before deleting it."); return; } const confirmation=window.prompt(`Permanent deletion removes all records for this lead. Type exactly: ${lead.companyName}`); if (confirmation !== lead.companyName) return; try { await deleteLead(lead.id, confirmation); await onSaved(); } catch (reason) { setError(String(reason)); } }
  return <div className="dialog-backdrop nested-dialog"><section className="dialog"><div className="dialog-heading"><div><p className="eyebrow">Lead record</p><h2>Edit lead</h2></div><button className="icon-button" onClick={onClose}><X size={18}/></button></div><form onSubmit={submit}><label>Company name<input name="companyName" required defaultValue={lead.companyName}/></label><label>Official website<input name="officialWebsite" required defaultValue={lead.officialWebsite}/></label><div className="form-grid"><label>Country<select name="country" defaultValue={lead.country}><option value="GB">United Kingdom</option><option value="US">United States</option><option value="FR">France</option></select></label><label>Language<select name="language" defaultValue={lead.language}><option value="en">English</option><option value="fr">French</option></select></label></div><label>City or service area<input name="cityOrServiceArea" defaultValue={lead.cityOrServiceArea}/></label>{error && <p className="form-error">{error}</p>}<div className="dialog-actions split-actions"><button type="button" className="danger-action" onClick={() => void remove()}>Delete lead</button><span/><button type="button" className="secondary-action" onClick={onClose}>Cancel</button><button className="primary-action" disabled={saving}>{saving ? "Saving…" : "Save changes"}</button></div></form></section></div>;
}

function FollowUpsView({ onChanged }: { onChanged: () => Promise<void> }) {
  const [items,setItems]=useState<FollowUpSummary[]>([]); const [error,setError]=useState<string>();
  const reload=useCallback(async()=>{ try { setItems(await getFollowUps()); setError(undefined); } catch(reason){ setError(String(reason)); } },[]);
  useEffect(()=>{ void reload(); },[reload]);
  async function act(action:()=>Promise<void>){ try { await action(); await Promise.all([reload(),onChanged()]); } catch(reason){ setError(String(reason)); } }
  return <section className="panel"><div className="panel-heading"><div><h3>Manual follow-ups</h3><p className="muted-copy">Created only after you confirm an email was sent.</p></div><span>{items.length} scheduled</span></div>{error&&<p className="form-error review-error">{error}</p>}{!items.length?<div className="compact-empty">No follow-ups scheduled.</div>:<div className="follow-up-list">{items.map(item=><article className="follow-up-row" key={item.id}><div><strong>{item.companyName}</strong><p>{item.recipient||"Recipient unavailable"} · {new Date(`${item.dueAt}Z`).toLocaleString()}</p></div><span className={`status status-${item.status}`}>{item.status}</span><button className="secondary-action" onClick={()=>void act(()=>snoozeFollowUp(item.id,3))}>Snooze 3 days</button><button className="primary-action" onClick={()=>void act(()=>markReplied(item.leadId))}>Mark replied</button></article>)}</div>}</section>;
}
...
function SettingsView({ onNotice }: { onNotice: (message: string) => void }) {
  ...
  const reloadBackups=useCallback(async()=>setBackups(await listBackups()),[]);
  useEffect(() => { void Promise.all([getSettings().then(setSettings),reloadBackups()]).catch(reason => setError(String(reason))); }, [reloadBackups]);
  ...
  async function backup() { try { onNotice(`Backup created on D: at ${await createBackup()}`); await reloadBackups(); } catch (reason) { setError(String(reason)); } }
  async function restore(fileName:string) { const confirmation=window.prompt(`Restore ${fileName}? A safety snapshot is created first. Type RESTORE to continue.`); if (confirmation!=="RESTORE") return; try { await restoreBackup(fileName,confirmation); onNotice("Backup restored successfully. Reloading the workspace."); window.location.reload(); } catch(reason){ setError(String(reason)); } }
